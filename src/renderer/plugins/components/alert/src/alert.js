...
alert.install = function (Vue, options) {
    ...
    function vpAlert( content = '', title = '提示', confirm = "我再想想", close = "许下心愿",type = opt.type) {
        ...
        let tpl = new toastTpl(({
            propsData: { title, content, confirm, close, type }
        })).$mount().$el;
    ...
    Vue.prototype.$alert = async (content = '', title = '提示',confirm = "我再想想", close = "许下心愿", type = opt.type) => {
        let alertPlugins = await vpAlert(content, title, 'alert');
        return alertPlugins;
    }

    Vue.prototype.$alert.confirm = async (content = '', title = '提示', confirm = "我再想想", close = "许下心愿", type = opt.type) => {
        let alertPlugins = await vpAlert(content, title, confirm, close, 'confirm');
        return alertPlugins;
    }
